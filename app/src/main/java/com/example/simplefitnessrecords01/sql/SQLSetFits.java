...
public class SQLSetFits extends SQLiteOpenHelper {
    ...
    public void getTableInLog(String logTag, String selection){
        ...
        Cursor c = db.query(DATABASE_TABLE,null, COLUMN_INFO + " = ?",new String[]{selection},null,null,null);
    ...
    public void addRow(OneSet oneSet){
        ...
        cv.put(COLUMN_INFO, oneSet.getUniqueFitTraining());
    ...
    public void updateRow(String oldUniqueName , String newUniqueName) {
        ...
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_INFO, newUniqueName);

        //get cursor from table sql
        Cursor c = getWritableDatabase().query(DATABASE_TABLE,null, COLUMN_INFO + " = ?",new String[]{oldUniqueName},null,null,null);
        //put cursor to chosen position of recycler
        if(c.moveToFirst()) {
            //update the table in selected id parameter
            String whereClause = COLUMN_INFO + " = ?";
